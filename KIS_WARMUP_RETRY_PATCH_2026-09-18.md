...
### 0-1. 현재가 조회 지연: 실측 후 정정

공통 30초 제한시간이 지연의 원인이라고 판단해 현재가 제한시간을 8초로 줄였으나, 이후 실제 모의 현재가 조회가 15.172초 후 정상 응답하는 것을 확인함. 8초 제한은 정상 응답까지 중단하므로 수신 공백을 악화시킬 수 있었음. 기존 원인 단정 및 8초 설정을 정정함. 이 측정만으로 서버 내부와 네트워크 지연을 구분할 수는 없음.

- 웜업·잔고·미체결 등 일반 REST 제한시간은 30초로 유지.
- 현재가 조회의 별도 제한시간 기본값을 `KIS_QUOTE_TIMEOUT_SEC=30.0`으로 복구. 제한시간은 갱신 주기가 아님.
...
- 실측 후 정정: 어댑터·현재가 재시도·재연결 테스트 22건 통과. 실시간 수신 해결을 의미하지 않음.
- Python 문법 검사 통과.
